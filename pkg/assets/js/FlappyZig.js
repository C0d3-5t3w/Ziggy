...
(function (PowerupType) {
    PowerupType[PowerupType["SHRINK"] = 0] = "SHRINK";
    PowerupType[PowerupType["BULLET_BOOST"] = 1] = "BULLET_BOOST";
    PowerupType[PowerupType["INVINCIBLE"] = 2] = "INVINCIBLE";
    PowerupType[PowerupType["PIPE_BREAKER"] = 3] = "PIPE_BREAKER";
    PowerupType[PowerupType["ENEMY_KILLER"] = 4] = "ENEMY_KILLER";
    PowerupType[PowerupType["CHEESE_HEAL"] = 5] = "CHEESE_HEAL";
})(PowerupType || (PowerupType = {}));
class Game {
    constructor() {
        this.collisionSlowdownFactor = 0.4;
        this.cheeseSpawnMin = 300;
        this.cheeseSpawnMax = 800;
        this.cheeseHealAmount = 0.5;
        this.powerupSpawnTimer = 0;
        this.powerupSpawnInterval = 200;
    ...
    calculateDifficultyMultipliers() {
        ...
        this.speedMultiplier = Math.min(1 + (level * CONSTANTS.SPEED_INCREASE_FACTOR), CONSTANTS.MAX_SPEED_MULTIPLIER);
        this.currentBulletSize = Math.min(CONSTANTS.BULLET_SIZE + (level * CONSTANTS.BULLET_SIZE_INCREASE), CONSTANTS.MAX_BULLET_SIZE);
    }
    getRandomWindInterval() {
        return Math.floor(Math.random() *
            (CONSTANTS.BASE_WIND_GUST_INTERVAL_MAX - CONSTANTS.BASE_WIND_GUST_INTERVAL_MIN) +
            CONSTANTS.BASE_WIND_GUST_INTERVAL_MIN);
    }
    getRandomCheeseInterval() {
    ...
    takeDamage(amount = 1) {
        if (this.immunityFrames > 0 || this.hasPowerup(PowerupType.INVINCIBLE))
            return;
        this.health -= amount;
    ...
    spawnPowerup() {
        if (Math.random() > CONSTANTS.POWERUP_SPAWN_CHANCE * 2)
            return;
    ...
    update() {
        ...
        this.bullets.forEach(bullet => {
            ...
            if (bullet.colorPhase >= Math.PI * 2)
                bullet.colorPhase = 0;
        });
        this.bullets = this.bullets.filter(bullet => bullet.x < CONSTANTS.SCREEN_WIDTH &&
            bullet.y > 0 &&
    ...
    draw() {
        ...
        if (this.trail.length >= 2) {
            for (let i = 1; i < this.trail.length; i++) {
                ...
                this.ctx.beginPath();
                this.ctx.moveTo(this.trail[i - 1].x + CONSTANTS.ZIG_WIDTH / 2, this.trail[i - 1].y + CONSTANTS.ZIG_HEIGHT / 2);
                this.ctx.lineTo(this.trail[i].x + CONSTANTS.ZIG_WIDTH / 2, this.trail[i].y + CONSTANTS.ZIG_HEIGHT / 2);
                this.ctx.stroke();
                this.ctx.restore();
            }
        }
        ...
        this.ctx.shadowColor = `rgb(${r}, ${g}, ${b})`;
        this.ctx.drawImage(this.zigImg, this.zig.x, this.zig.y, CONSTANTS.ZIG_WIDTH, CONSTANTS.ZIG_HEIGHT);
        this.ctx.restore();
        this.walls.forEach(wall => {
            if (wall.isMoving) {
        ...
        this.bullets.forEach(bullet => {
            ...
            if (this.meowImg.complete) {
                ...
                this.ctx.drawImage(this.meowImg, bullet.x, bullet.y, this.currentBulletSize, this.currentBulletSize);
                this.ctx.restore();
            }
            else {
                this.ctx.strokeStyle = `rgb(${r * 255},${g * 255},${b * 255})`;
                this.ctx.beginPath();
                this.ctx.arc(bullet.x + this.currentBulletSize / 2, bullet.y + this.currentBulletSize / 2, this.currentBulletSize / 2, 0, Math.PI * 2);
                this.ctx.stroke();
            }
        });
        this.enemies.forEach(enemy => {
            ...
            switch (enemy.shape) {
                case 0:
                    if (this.stopImg.complete) {
                        ...
                        this.ctx.shadowBlur = 20;
                        this.ctx.shadowColor = `rgb(${r * 255},${g * 255},${b * 255})`;
                        this.ctx.drawImage(this.stopImg, enemy.x, enemy.y, size, size);
                        this.ctx.restore();
                    }
                    else {
                        this.ctx.beginPath();
                ...
                case 1:
                    if (this.noImg.complete) {
                        ...
                        this.ctx.shadowBlur = 20;
                        this.ctx.shadowColor = `rgb(${r * 255},${g * 255},${b * 255})`;
                        this.ctx.drawImage(this.noImg, enemy.x, enemy.y, size, size);
                        this.ctx.restore();
                    }
                    else {
                        this.ctx.beginPath();
                ...
                case 2:
                    if (this.downImg.complete) {
                        ...
                        this.ctx.shadowBlur = 20;
                        this.ctx.shadowColor = `rgb(${r * 255},${g * 255},${b * 255})`;
                        this.ctx.drawImage(this.downImg, enemy.x, enemy.y, size, size);
                        this.ctx.restore();
                    }
                    else {
                        this.ctx.beginPath();
                ...
                case 3:
                    if (this.badImg.complete) {
                        ...
                        this.ctx.shadowBlur = 20;
                        this.ctx.shadowColor = `rgb(${r * 255},${g * 255},${b * 255})`;
                        this.ctx.drawImage(this.badImg, enemy.x, enemy.y, size, size);
                        this.ctx.restore();
                    }
                    else {
                        this.ctx.beginPath();
        ...
        if (this.windActive) {
            ...
            for (const particle of this.windParticles) {
                ...
                this.ctx.lineTo(particle.x - this.windDirection * (5 + particle.speed * 2) * CONSTANTS.SCALE_FACTOR, particle.y);
                this.ctx.stroke();
                this.ctx.beginPath();
    ...
    drawUI() {
        ...
        if (this.gameOver) {
            ...
            if (CONSTANTS.IS_MOBILE && !localStorage.getItem('controlsShown') && this.score < 2) {
                ...
                this.ctx.fillRect(hintX - 10, CONSTANTS.SCREEN_HEIGHT - 40 * CONSTANTS.SCALE_FACTOR - 10, hintMetrics.width + 20, 30 * CONSTANTS.SCALE_FACTOR);
                this.ctx.fillStyle = 'rgba(0, 0, 0, 0.8)';
                this.ctx.fillText(controlsHint, hintX, CONSTANTS.SCREEN_HEIGHT - 20 * CONSTANTS.SCALE_FACTOR);
                if (this.score >= 1) {
    ...
    drawHealthBar() {
        ...
        this.ctx.fillRect(barX + 2, barY + 2, (CONSTANTS.BASE_HEALTH_BAR_WIDTH - 4) * healthRatio, CONSTANTS.BASE_HEALTH_BAR_HEIGHT - 4);
        this.ctx.strokeStyle = 'white';
        this.ctx.lineWidth = 2;
    ...
    drawActivePowerups() {
        ...
        if (this.activePowerups.length > 0) {
            ...
            this.ctx.fillRect(10, startY - ((this.activePowerups.length - 1) * (iconSize + 5)) - iconSize, iconSize + 10, this.activePowerups.length * (iconSize + 5));
        }
    ...
    checkEnemyCollision(enemy) {
        ...
        const enemyCenterX = enemy.x + CONSTANTS.ENEMY_SIZE / 2;
        const enemyCenterY = enemy.y + CONSTANTS.ENEMY_SIZE / 2;
        const distance = Math.sqrt(Math.pow(zigCenterX - enemyCenterX, 2) +
    ...
    checkBulletEnemyCollision(bullet, enemy) {
        ...
        const enemyCenterX = enemy.x + CONSTANTS.ENEMY_SIZE / 2;
        const enemyCenterY = enemy.y + CONSTANTS.ENEMY_SIZE / 2;
        const distance = Math.sqrt(Math.pow(bulletCenterX - enemyCenterX, 2) +
    ...
    checkEnemyTrailCollision(enemy) {
        ...
        for (let i = 1; i < this.trail.length; i++) {
            ...
            const distance = this.pointLineDistance(enemyCenterX, enemyCenterY, trailX1, trailY1, trailX2, trailY2);
    ...
    checkPowerupCollision(powerup) {
        ...
        const distance = Math.sqrt(Math.pow(zigCenterX - powerupCenterX, 2) +
...
// <3
//# sourceMappingURL=FlappyZig.js.map