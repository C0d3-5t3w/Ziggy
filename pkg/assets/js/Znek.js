...
class Znek {
    constructor() {
        this.canvas = document.getElementById('znekCanvas');
    ...
    updateLayout() {
        if (!this.canvas || !this.dpadElement)
            return;
        ...
        if (this.dpadElement) {
            ...
            buttons.forEach(button => {
                button.style.fontSize = `${Math.max(16, Math.floor(dpadSize / 6))}px`;
            });
            const shootButton = this.dpadElement.querySelector('.shoot-button');
            if (shootButton) {
        ...
        if (this.restartButtonElement) {
            const buttonWidth = Math.min(CONSTANTS.RESTART_BUTTON_WIDTH, windowWidth * 0.3);
    ...
    scheduleSpecialFood() {
        ...
        this.specialFoodTimeout = window.setTimeout(() => {
            ...
            do {
                ...
                onSnakeOrFood = this.snake.some(segment => segment.x === newSpecialFood.x && segment.y === newSpecialFood.y) ||
            ...
            this.specialFood = Object.assign(Object.assign({}, newSpecialFood), { timeLeft: CONSTANTS.SPECIAL_FOOD_DURATION });
            window.setTimeout(() => {
                this.specialFood = null;
                this.scheduleSpecialFood();
    ...
    displayTailEatingNotification(duration) {
    ...
    spawnGhost() {
        ...
        do {
            ...
            const distance = Math.sqrt(Math.pow(x - head.x, 2) + Math.pow(y - head.y, 2));
    ...
    moveGhosts() {
        if (!this.gameStarted || this.ghosts.length === 0)
            return;
        const head = this.snake[0];
        this.ghosts.forEach(ghost => {
            ...
            if (distance > 0) {
                ...
                const wouldHitBody = this.snake.some((segment, i) => i > 0 &&
                ...
                if (wouldHitBody) {
                    if (Math.abs(dx) > Math.abs(dy)) {
                        ...
                        newY = ghost.y;
                        const horizontalHitsBody = this.snake.some((segment, i) => i > 0 &&
                            Math.abs(segment.x - newX) < CONSTANTS.GRID_SIZE / 2 &&
                            Math.abs(segment.y - newY) < CONSTANTS.GRID_SIZE / 2);
                        ...
                        newX = ghost.x;
                        newY = ghost.y + Math.sign(dy) * (CONSTANTS.GRID_SIZE / 4);
                        const verticalHitsBody = this.snake.some((segment, i) => i > 0 &&
                            Math.abs(segment.x - newX) < CONSTANTS.GRID_SIZE / 2 &&
                            Math.abs(segment.y - newY) < CONSTANTS.GRID_SIZE / 2);
                        if (verticalHitsBody) {
                            newX = ghost.x + Math.sign(dx) * (CONSTANTS.GRID_SIZE / 4);
    ...
    update() {
        ...
        if (this.gameStarted) {
            ...
            if (head.x < 0)
                head.x = this.canvas.width - CONSTANTS.GRID_SIZE;
            if (head.x >= this.canvas.width)
                head.x = 0;
            if (head.y < 0)
                head.y = this.canvas.height - CONSTANTS.GRID_SIZE;
            if (head.y >= this.canvas.height)
                head.y = 0;
            if (this.hasTailEatingPower) {
                const collisionIndex = this.snake.findIndex((segment, i) => i > 2 && segment.x === head.x && segment.y === head.y);
                if (collisionIndex > 2) {
            ...
            const headToFoodDistance = Math.sqrt(Math.pow(headCenterX - foodCenterX, 2) +
            ...
            if (headToFoodDistance < collisionRadius) {
                ...
                const headToSpecialFoodDistance = Math.sqrt(Math.pow(headCenterX - specialFoodCenterX, 2) +
    ...
    moveBullets() {
        ...
        this.bullets = this.bullets.filter(bullet => bullet.distanceTraveled < CONSTANTS.BULLET_RANGE &&
            bullet.x >= 0 &&
    ...
    checkBulletGhostCollisions() {
        if (this.bullets.length === 0 || this.ghosts.length === 0)
            return;
    ...
    draw() {
        ...
        for (let i = this.snake.length - 1; i > 0; i--) {
            ...
            this.roundedRect(segment.x + (CONSTANTS.SNAKE_BODY_SIZE - segmentSize) / 2, segment.y + (CONSTANTS.SNAKE_BODY_SIZE - segmentSize) / 2, segmentSize, segmentSize, Math.min(6, Math.max(3, i / 3)));
            this.ctx.fill();
            this.ctx.fillStyle = `hsla(${hue}, ${CONSTANTS.SNAKE_BODY_SATURATION}%, ${CONSTANTS.SNAKE_BODY_LIGHTNESS + 30}%, 0.5)`;
            this.ctx.beginPath();
            this.ctx.arc(segment.x + CONSTANTS.SNAKE_BODY_SIZE / 2, segment.y + CONSTANTS.SNAKE_BODY_SIZE / 2, segmentSize / 3, 0, Math.PI * 2);
            this.ctx.fill();
            this.ctx.restore();
        }
        if (this.snakeImg.complete) {
            ...
            this.ctx.drawImage(this.snakeImg, head.x - offsetX, head.y - offsetX, CONSTANTS.SNAKE_HEAD_SIZE, CONSTANTS.SNAKE_HEAD_SIZE);
            this.ctx.restore();
        }
        this.ctx.save();
        ...
        this.drawFish(this.food.x + CONSTANTS.FOOD_SIZE / 2, this.food.y + CONSTANTS.FOOD_SIZE / 2, CONSTANTS.FOOD_SIZE * regularFoodPulse);
        this.ctx.restore();
        if (this.specialFood) {
            ...
            this.drawSpecialFoodParticles(this.specialFood.x + CONSTANTS.SPECIAL_FOOD_SIZE / 2, this.specialFood.y + CONSTANTS.SPECIAL_FOOD_SIZE / 2, CONSTANTS.SPECIAL_FOOD_SIZE);
            this.drawFish(this.specialFood.x + CONSTANTS.SPECIAL_FOOD_SIZE / 2, this.specialFood.y + CONSTANTS.SPECIAL_FOOD_SIZE / 2, actualSize);
        ...
        this.ghosts.forEach(ghost => {
            ...
            const ghostGradient = this.ctx.createRadialGradient(ghost.x + CONSTANTS.GHOST_SIZE / 2, ghost.y + CONSTANTS.GHOST_SIZE / 2, 0, ghost.x + CONSTANTS.GHOST_SIZE / 2, ghost.y + CONSTANTS.GHOST_SIZE / 2, CONSTANTS.GHOST_SIZE);
            ...
            this.ctx.shadowBlur = 20;
            this.drawGhost(ghost.x + CONSTANTS.GHOST_SIZE / 2, ghost.y + CONSTANTS.GHOST_SIZE / 2, pulseSize);
        ...
        this.bullets.forEach(bullet => {
            this.ctx.save();
            const bulletGradient = this.ctx.createRadialGradient(bullet.x + CONSTANTS.BULLET_SIZE / 2, bullet.y + CONSTANTS.BULLET_SIZE / 2, 0, bullet.x + CONSTANTS.BULLET_SIZE / 2, bullet.y + CONSTANTS.BULLET_SIZE / 2, CONSTANTS.BULLET_SIZE / 2);
            ...
            this.ctx.shadowBlur = 15;
            this.ctx.beginPath();
            this.ctx.arc(bullet.x + CONSTANTS.BULLET_SIZE / 2, bullet.y + CONSTANTS.BULLET_SIZE / 2, CONSTANTS.BULLET_SIZE / 2, 0, Math.PI * 2);
            this.ctx.fill();
            ...
            const trailGradient = this.ctx.createLinearGradient(bullet.x + CONSTANTS.BULLET_SIZE / 2, bullet.y + CONSTANTS.BULLET_SIZE / 2, trailX, trailY);
        ...
        if (this.showNotification) {
            ...
            const gradient = this.ctx.createLinearGradient(0, notificationY - fontSize, 0, notificationY);
        ...
        this.ctx.fillText(`SCORE: ${this.score}`, CONSTANTS.SCORE_POSITION_X, CONSTANTS.SCORE_POSITION_Y);
        ...
        if (this.gameOver) {
            this.ctx.save();
            const overlay = this.ctx.createRadialGradient(this.canvas.width / 2, this.canvas.height / 2, 0, this.canvas.width / 2, this.canvas.height / 2, this.canvas.width);
            ...
            this.ctx.shadowBlur = 15;
            const gameOverGradient = this.ctx.createLinearGradient(0, 70, 0, 90);
            ...
            this.highScores.forEach((score, i) => {
                if (score === this.score && this.deathTimer < CONSTANTS.DEATH_PAUSE * 2) {
                    ...
                    this.ctx.lineWidth = 2;
                    this.roundedRect(this.canvas.width / 2 - 80, CONSTANTS.HIGH_SCORE_Y_START + (i * CONSTANTS.HIGH_SCORE_Y_SPACING) - 22, 160, 30, 5);
                    this.ctx.stroke();
                    this.ctx.restore();
                }
                else {
                ...
                this.ctx.fillText(`${i + 1}. ${score}`, this.canvas.width / 2, CONSTANTS.HIGH_SCORE_Y_START + (i * CONSTANTS.HIGH_SCORE_Y_SPACING));
            });
            if (this.canReset) {
                this.ctx.save();
            ...
            const gradient = this.ctx.createLinearGradient(0, this.canvas.height / 2 - 15, 0, this.canvas.height / 2 + 15);
    ...
    drawSpecialFoodParticles(x, y, size) {
...
exports.default = Znek;
// <3
//# sourceMappingURL=Znek.js.map